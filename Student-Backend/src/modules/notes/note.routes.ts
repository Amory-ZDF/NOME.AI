...
const notePatchBodySchema = z.object({
  ...
  changedAt: z.unknown().optional(), updatedAt: z.unknown().optional(), reason: z.unknown().optional(),
}).passthrough()
...
export async function noteRoutes(app: FastifyInstance, options: NoteRoutesOptions): Promise<void> {
  ...
  routes.post('/api/notes', { schema: { tags: ['notes'], summary: 'Create a versioned note', body: createNoteBodySchema, response: { 200: noteEnvelopeSchema, ...errors } } }, async (request) => ok({ note: await service.create(request.body) }))
  routes.patch('/api/notes/:id', { schema: { tags: ['notes'], summary: 'Edit a note and record its prior version', params: noteParamsSchema, body: notePatchBodySchema, response: { 200: noteEnvelopeSchema, ...errors } } }, async (request) => ok({ note: await service.update(request.params.id, request.body) }))
}
