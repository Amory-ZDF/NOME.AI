...
function servicesFor({ completeTask = vi.fn(async (id) => ({ task: { ...tasks.find((task) => task.id === id), status: 'completed', completedAt: now.toISOString(), isOverdue: false } })) } = {}) {
  return createAppServices({
    apiClient: {
      bootstrap: async () => ({ tasks, taskAdjustments: [], greeting: null, moduleStats: null, learningSummary: { weakTopics: ['calculus'], knowledgeHeatmap: [] }, errors: [], notes: [], noteFolders: [], settings: {} }),
      completeTask,
